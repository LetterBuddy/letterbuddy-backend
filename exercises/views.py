    ...
    def put(self, request, pk):
        ...
        try:
            ...
            qwen2_guess = client.predict(
                    ...
                    text_input="just write what you recognized without any more words or symbols(no punctuation)",
                    api_name="/qwen_inference",
            )
            # remove ""
            qwen2_guess = qwen2_guess.replace('"', '')
            print(qwen2_guess)
        ...
        if results[0] is not None or qwen2_guess != "":
            ...
            for i in range(len(exercise.requested_text)):
                ...
                char = ''
                char_conf = 0.0
                # qwen2 guessed something for that letter
                if i < len(qwen2_guess):
                        qwen2_char_guess = qwen2_guess[i]
                        print(f"Qwen2-VL-2B detected: {qwen2_char_guess}")
                        char_conf += 1.0 # can be played with - the idea is to give more weight to the qwen2 guess
                        # if the guess is correct - save it
                        if exercise.requested_text[i] == qwen2_char_guess:
                            char = qwen2_guess[i]
                ...
                if results[0] and i < len(results[0][0][1]):
                    ...
                    if qwen2_char_guess == paddle_char_guess:
                ...
                if char != '':
                    ...
                    # prefer qwen2 guess if it is not empty
                    char = qwen2_char_guess if qwen2_char_guess != '' else paddle_char_guess
